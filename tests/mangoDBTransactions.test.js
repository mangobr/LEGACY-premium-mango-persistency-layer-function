...
describe("Should test mongoDB ScannedFood Collection transactions", () => {
  ...
  test("should insert a consolidated food scan in MongoDB", async () => {
    const insertedConsolidatedFoodScan = await insertValidatedFoodAssignment(
      returnConsolidatedFoodScan()
    );
    expect(insertedConsolidatedFoodScan).toEqual(
      "Scan consolidado populado no MangoDB"
    );
  });
});
